...
(function ($) {
    var isAndroid = window.navigator.userAgent.toLowerCase().indexOf("android") > -1,
        pasteEventName = ($.browser.msie ? 'paste' : 'input') + ".ipinput";;
        ...
        ipAddress:function (s) {
            ...
            return $(this).each(function () {
                ...
                if (ctx.input.val() == '' || !isIp(ctx.input.val()))
                    ctx.input.val('');
                var maxLen = isAndroid?(s.v == 4 ? 16 : 40):(s.v == 4 ? 15 : 39);
                ctx.input.attr('maxlength', maxLen).attr('size', maxLen);
                ...
                ctx.input.bind('blur.ipinput',function () {
                    ...
                    .bind('keydown.ipinput',keydownEvent)
                    .bind('keypress.ipinput',keypressEvent)
                    .bind(pasteEventName, function (e) {
                        ctx.input.val('');
                        setTimeout(function () {
                            ctx.input.blur();
                        }, 0);
